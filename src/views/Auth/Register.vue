<template>
    <AuthBaseTemplate>
        <template #title>Créer votre votre compte</template>
        <template #default>
            <form class="space-y-6" action="#" method="POST" @submit.prevent="handleSubmit">
                <div>
                    <label for="email" class="block text-sm/6 font-medium text-gray-900">Nom et prénom</label>
                    <div class="mt-2">
                        <input type="text" name="name" id="name" autocomplete="name" v-model="user.name"
                        ...
                        <p v-if="errors.name != undefined" class="text-red-500 text-sm font-semibold">{{ errors.name[0]
                            }}</p>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            <label for="password" class="block text-sm/6 font-medium text-gray-900">Vous avez déjà
                                compte ?</label>
                            <div class="text-sm">
                ...
                <div>
                    <Button :loading="isLoading" :text="'Créer son compte'" type-name="submit"></Button>
                </div>
            </form>
        </template>
    </AuthBaseTemplate>
...
const handleSubmit = () => {
    ...
    httpClient.get('sanctum/csrf-cookie').then(() => {
        httpClient.post("api/register", user.value)
